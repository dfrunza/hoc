...
execute_instr(HocMachine* machine, Instruction* instr)
{
  ...
  switch(opcode)
  {
      ...
      if(check_sp_bounds(machine, arg_sp))
      {
        switch(opcode)
        {
          ...
          case Opcode_MOD_INT:
          {
            ...
            int32 result = 0;
            if(opcode == Opcode_ADD_INT)
              result = arg1 + arg2;
            else if(opcode == Opcode_SUB_INT)
              result = arg1 - arg2;
            else if(opcode == Opcode_MUL_INT)
              result = arg1 * arg2;
            else if(opcode == Opcode_MOD_INT)
            {
              if(arg2 != 0)
                result = arg1 % arg2;
              else
          ...
          case Opcode_DIV_FLOAT:
          {
            ...
            float32 result = 0;
            if(opcode == Opcode_ADD_FLOAT)
              result = arg1 + arg2;
            else if(opcode == Opcode_SUB_FLOAT)
              result = arg1 - arg2;
            else if(opcode == Opcode_MUL_FLOAT)
              result = arg1 * arg2;
            else if(opcode == Opcode_DIV_FLOAT)
            {
              if(arg2 != 0)
                result = arg1 / arg2;
              else
    ...
    case Opcode_NEG_FLOAT:
    {
      ...
      if(check_sp_bounds(machine, arg_sp))
      {
        if(opcode == Opcode_NEG_INT)
        {
          int32 arg = memory_at(arg_sp, int32, 0);
          memory_at(arg_sp, int32, 0) = -arg;
        }
        else if(opcode == Opcode_NEG_FLOAT)
        {
          float32 arg = memory_at(arg_sp, float32, 0);
        ...
        else if(instr->param.int_val == 1)
        {
          ...
          if(check_sp_bounds(machine, arg_sp))
          {
            ...
            if(check_memory_bounds(machine, location))
            {
              ...
              machine->sp = location_at(machine->sp, int8, -3);
              machine->ip++;
            }
            else
              return Result_InvalidMemoryAccess;
    ...
    case Opcode_STORE:
    {
      if(instr->param_type == ParamType_Int32)
      {
        if(instr->param.int_val == 4)
        {
          if(check_sp_bounds(machine, location_at(machine->sp, int32, -2)))
          {
        ...
        {
          if(check_sp_bounds(machine, location_at(machine->sp, int8, -5)))
          {
            int32 location = memory_at(machine->sp, int32, -1);
            if(check_memory_bounds(machine, location))
            {
    ...
    case Opcode_DUP:
    {
      if(check_sp_bounds(machine, location_at(machine->sp, int32, -1)))
      {
