...
bool name_resolve(AstNode* node, List* fwd_occurs);

bool name_resolve_type(AstNode* node, List* fwd_occurs)
{
  ...
  else if(node->kind == AstNode_pointer)
  {
    ...
    if(success = name_resolve_type(ATTR(&ptr_gen0, ast_node, type_expr), fwd_occurs))
    {
  ...
  else if(node->kind == AstNode_array)
  {
    ...
    if(success = name_resolve_type(ATTR(&array_gen0, ast_node, type_expr), fwd_occurs) &&
        name_resolve(ATTR(&array_gen0, ast_node, size_expr), fwd_occurs))
    {
...
bool name_resolve_block(AstNode* node, List* fwd_occurs)
{
  ...
  for(ListItem* list_item = ATTR(&block_copy, list, nodes)->first;
      ...
      list_item = list_item->next)
  {
    success = name_resolve(ITEM(list_item, ast_node), fwd_occurs);
  }
...
bool name_resolve(AstNode* node, List* fwd_occurs)
{
  ...
  if(node->kind == AstNode_module)
  {
    ...
    if(success = name_resolve_block(body, fwd_occurs))
    {
  ...
  else if(node->kind == AstNode_bin_expr)
  {
    ...
    if(success = name_resolve(left_operand, fwd_occurs) && name_resolve(right_operand, fwd_occurs))
    {
  ...
  else if(node->kind == AstNode_un_expr)
  {
    ...
    if(success = name_resolve(operand, fwd_occurs))
    {
  ...
  else if(node->kind == AstNode_block)
  {
    ...
    success = name_resolve_block(node, fwd_occurs);
    end_scope();
  }
  else if(node->kind == AstNode_stmt)
  {
    ...
    if(success = name_resolve(ATTR(&stmt_gen0, ast_node, stmt), fwd_occurs))
    {
  ...
  else if(node->kind == AstNode_if_stmt)
  {
    ...
    if(success = name_resolve(cond_expr, fwd_occurs))
    {
      ...
      success = name_resolve(body, fwd_occurs);
      end_scope();
      ...
      if(success)
      {
        ...
        if(else_body)
        {
          ...
          begin_scope(ScopeKind_block, if_stmt_gen1);
          success = name_resolve_block(else_body, fwd_occurs);
          end_scope();
  ...
  else if(node->kind == AstNode_while_stmt)
  {
    ...
    if(success = name_resolve(cond_expr, fwd_occurs))
    {
      ...
      success = name_resolve_block(body, fwd_occurs);
      end_scope();
  ...
  else if(node->kind == AstNode_return_stmt)
  {
    ...
    if(ret_expr && (success = name_resolve(ret_expr, fwd_occurs)))
    {
  ...
  else if(node->kind == AstNode_type)
  {
    ...
    if(success = name_resolve_type(ATTR(&type_gen0, ast_node, type_expr), fwd_occurs))
    {
...
bool resolve_fwd_occurs(List* fwd_occurs)
{
  bool success = true;
  for(ListItem* list_item = fwd_occurs->first;
      list_item && success;
      list_item = list_item->next)
  {
    Symbol* occur_sym = ITEM(list_item, symbol);
    if(occur_sym->kind == Symbol_proc_occur)
    {
...
void build_occur_and_decl_types()
{
  for(ListItem* list_item = symbol_table->symbols->first;
      ...
      list_item = list_item->next)
  {
    Symbol* sym = ITEM(list_item, symbol);
    if(sym->kind == Symbol_var_decl)
    {
      AstNode* var_decl = sym->ast_node;
      AstNode* type_node = ATTR(var_decl, ast_node, type);
      ATTR(var_decl, type, type) = make_type_of_type_expr(ATTR(type_node, ast_node, type_expr));
    }
    else if(sym->kind == Symbol_proc_decl)
    {
      AstNode* proc_decl = sym->ast_node;
      AstNode* ret_type_node = ATTR(proc_decl, ast_node, ret_type);
      Type* ret_type = make_type_of_type_expr(ATTR(ret_type_node, ast_node, type_expr));

      Type* args_type = basic_type_void;
      ListItem* arg_list_item = ATTR(proc_decl, list, formal_args)->first;
      if(arg_list_item)
      {
      ...
      ATTR(proc_decl, type, type) = new_proc_type(args_type, ret_type);
    }
  }
  ...
  {
    Symbol* sym = ITEM(list_item, symbol);
    if(sym->kind == Symbol_var_occur)
    {
      AstNode* var_occur = sym->ast_node;
      AstNode* var_decl = SYM(sym, var_occur)->decl_sym->ast_node;
      ATTR(var_occur, type, type) = ATTR(var_decl, type, type);
    }
    else if(sym->kind == Symbol_proc_occur)
    {
      AstNode* proc_occur = sym->ast_node;
      AstNode* proc_decl = SYM(sym, proc_occur)->decl_sym->ast_node;
      ATTR(proc_occur, type, type) = ATTR(proc_decl, type, type);
    }
  }
}
...
bool typecheck(AstNode* node)
{
  ...
  else if(node->kind == AstNode_var_decl)
  {
    ...
    ATTR(node, type, type) = make_type_of_type_expr(ATTR(type_node, ast_node, type_expr));
  }
  return success;
}
