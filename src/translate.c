...
Symbol* new_temp_object(MemoryArena* arena, Scope* scope, Type* ty)
{
...
void ir_emit_assign(IrContext* ir_context, eIrOp op, eBasicType op_type, IrArg* arg1, IrArg* arg2, IrArg* result)
{
...
bool gen_ir_bin_expr(IrContext* ir_context, Scope* scope, AstNode* bin_expr)
{
  ...
  switch(op)
  {
    ...
    if(success = gen_ir_expr(ir_context, scope, left_operand) && gen_ir_expr(ir_context, scope, right_operand))
    {
      ...
      place->object = new_temp_object(ir_context->sym_arena, scope, bin_expr->eval_ty);
...
bool gen_ir_unr_expr(IrContext* ir_context, Scope* scope, AstNode* unr_expr)
{
  ...
  switch(op)
  {
    ...
    if(success = gen_ir_expr(ir_context, scope, operand))
    {
      ...
      place->kind = eIrArg_object;
      place->object = new_temp_object(ir_context->sym_arena, scope, unr_expr->eval_ty);

      assert(operand->eval_ty->kind == eType_basic);
      eBasicType op_type = KIND(operand->eval_ty, eType_basic)->basic.kind;
      ir_emit_assign(ir_context, conv_operator_to_ir_op(op), op_type, operand->place, 0, unr_expr->place);
    }
    break;
...
void gen_ir_call(IrContext* ir_context, Scope* scope, AstNode* call)
{
  ...
  place->kind = eIrArg_object;
  place->object = new_temp_object(ir_context->sym_arena, scope, call->eval_ty);
...
bool gen_ir_index(IrContext* ir_context, Scope* scope, AstNode* index)
{
  ...
  else if(array_expr->kind == eAstNode_index)
  {
    if(success = gen_ir_index(ir_context, scope, array_expr) && gen_ir_expr(ir_context, scope, i_expr))
    {
      ...
      t->object = new_temp_object(arena, scope, basic_type_int);
      ...
      if(dim_size->constant.int_val = size_of_array_dim(index->index.array_ty, index->index.ndim) > 0)
      {
        ...
        ir_emit_assign(ir_context, eIrOp_add, eBasicType_int, t, i_expr->place, t);
      }
      else
...
bool gen_ir_index_with_offset(IrContext* ir_context, Scope* scope, AstNode* index)
{
  ...
  if(success = gen_ir_index(ir_context, scope, index))
  {
    ...
    offset->object = new_temp_object(arena, scope, basic_type_int);
...
bool gen_ir_cast(IrContext* ir_context, Scope* scope, AstNode* cast)
{
  ...
  if(success = gen_ir_expr(ir_context, scope, from_expr))
  {
    ...
    if(require_conv)
    {
      ...
      place->kind = eIrArg_object;
      place->object = new_temp_object(arena, scope, cast->eval_ty);
      ...
      ir_emit_assign(ir_context, cast_op, eBasicType_None, from_expr->place, 0, cast->place);
    }
  }
  return success;
}
  ...
  switch(expr->kind)
  {
    case eAstNode_bin_expr:
    {
      ...
      if(op == eOperator_logic_and || op == eOperator_logic_or)
      {
        ...
        place->kind = eIrArg_object;
        place->object = new_temp_object(ir_context->sym_arena, scope, expr->eval_ty);
        ...
        ir_emit_assign(ir_context, eIrOp_None, eBasicType_None, &ir_arg_bool_false, 0, expr->place);
    ...
    case eAstNode_index:
    {
      if(success = gen_ir_index_with_offset(ir_context, scope, expr))
      {
        IrArg* place = expr->place = mem_push_struct(arena, IrArg);
        place->kind = eIrArg_object;
        place->object = new_temp_object(arena, scope, expr->eval_ty);
...
{
  assert(KIND(var, eAstNode_var));
  alloc_data_object(var->var.decl_sym, scope);
}
...
void x86_print_operand(String* text, X86Operand* operand)
{
  switch(operand->kind)
  {
    ...
    {
      str_printf(text, "dword ptr [");
      x86_print_operand(text, operand->indexed.base);
      if(operand->indexed.offset)
      {
        str_printf(text, "+");
        x86_print_operand(text, operand->indexed.offset);
      }
      str_printf(text, "]");
    }
    break;
    ...
    {
      str_printf(text, "%d", operand->constant);
    }
    break;
...
{
  X86Operand* operand = mem_push_struct(arena, X86Operand);
  if(constant->kind == eIrConstant_int)
  {
    operand->kind = eX86Operand_constant;
    operand->constant = constant->int_val;
  }
  else
    assert(0);
  return operand;
...
X86Operand* x86_make_memory_operand(MemoryArena* arena, Symbol* object)
{
  ...
  if(object->storage_space == eStorageSpace_stack)
  {
    ...
    operand->memory.offset = -(object->data_loc + MACHINE_WORD_SIZE); // X86 stack grows downwards
  }
...
void x86_emit_load_constant_into_register(X86Context* context, eX86Location dest_reg, IrConstant* constant)
{
  assert(is_register_location(dest_reg));
...
void x86_emit_store_constant_to_memory(X86Context* context, IrConstant* constant, Symbol* object)
{
  X86Stmt* stmt = x86_new_stmt(context, eX86StmtOpcode_mov);
...
void x86_emit_store_object_to_memory(X86Context* context, eX86Location source_reg, Symbol* object)
{
...
void x86_emit_load_memory_address(X86Context* context, eX86Location dest_reg, Symbol* object)
{
  assert(is_register_location(dest_reg));
...
eX86StmtOpcode conv_ir_op_to_x86_opcode(eIrOp ir_op, eBasicType op_type)
{
  ...
  switch(ir_op)
  {
    case eIrOp_add:
    {
      if(op_type == eBasicType_int)
      {
        x86_opcode = eX86StmtOpcode_add;
      }
      else if(op_type == eBasicType_float)
      {
...
eX86Location find_min_used_register(X86Context* context, Symbol* exclude_object)
{
...
void save_object_to_memory(X86Context* context, eX86Location reg, Symbol* object)
{
  assert(is_register_location(reg));

  if(!is_object_in_memory(object))
  {
    x86_emit_store_object_to_memory(context, reg, object);
...
eX86Location get_best_available_register(X86Context* context, IrStmt* stmt)
{
    ...
    else
    {
      if(stmt->kind == eIrStmt_assign)
      {
        ...
        else
        {
          ...
          best_reg = find_min_used_register(context, exclude_object);
          save_register_to_memory(context, best_reg, true);
        }
      }
      else if(stmt->kind == eIrStmt_cond_goto)
      {
        best_reg = find_min_used_register(context, 0);
        save_register_to_memory(context, best_reg, true);
      }
      else
        assert(0);
...
void discard_all_assign_args(X86Context* context, struct IrStmt_assign* assign, eX86Location store_reg)
{
  if(assign->arg1->kind == eIrArg_object)
  {
    ...
    if(arg1_loc == store_reg)
    {
      delete_object_from_location(context, assign->arg1->object, arg1_loc);
    }
    else
    {
...
eX86Location select_result_register(X86Context* context, IrStmt* stmt)
{
  ...
  if(stmt->kind == eIrStmt_assign)
  {
    ...
    if(assign->op)
    {
      if(is_ir_op_arithmetic(assign->op))
      {
        if(assign->arg1->kind == eIrArg_object)
        {
          eX86Location arg1_loc = lookup_object_location(assign->arg1->object);
          if(is_register_location(arg1_loc)
             && is_single_occupant_register(context, arg1_loc, assign->arg1->object)
             && (assign->arg1->next_use == NextUse_None && !assign->arg1->is_live))
          {
            dest_loc = arg1_loc;
          }
          else
          {
            dest_loc = get_best_available_register(context, stmt);
          }
        }
        else if(assign->arg1->kind == eIrArg_constant)
        {
          eX86Location result_loc = lookup_object_location(assign->result->object);
          if(is_register_location(result_loc)
             && is_single_occupant_register(context, result_loc, assign->result->object))
    ...
    else // !assign.op
    {
      ...
      {
        if(assign->arg1->kind == eIrArg_constant
           && assign->result->next_use == NextUse_None)
        {
  ...
  else if(stmt->kind == eIrStmt_cond_goto)
  {
    ...
    if(cond_goto->arg1->kind == eIrArg_object)
    {
      eX86Location arg1_loc = lookup_object_location(cond_goto->arg1->object);
      if(is_register_location(arg1_loc))
      {
        dest_loc = arg1_loc;
      }
...
void set_result_next_use_info(IrArg* result)
{
  Symbol* object = KIND(result, eIrArg_object)->object;
  result->is_live = object->is_live;
  result->next_use = object->next_use;
  object->is_live = object->is_temp ? false : true;
  object->next_use = NextUse_None;
}

void set_arg_next_use_info(IrArg* arg, NextUse next_use)
{
  Symbol* object = arg->object;
  arg->is_live = object->is_live;
  arg->next_use = object->next_use;
  object->is_live = true;
  object->next_use = next_use;
}
...
bool translate(char* title, char* file_path, char* hoc_text, String* x86_text)
{
        ...
        li = li->next)
    {
      ...
      for(int i = bb->stmt_count - 1; i >=0 ; i--)
      {
        ...
        if(stmt->kind == eIrStmt_assign)
        {
          ...
          set_result_next_use_info(assign->result);

          if(assign->arg1->kind == eIrArg_object)
          {
            set_arg_next_use_info(assign->arg1, i);
          }

          if(assign->arg2 && assign->arg2->kind == eIrArg_object)
          {
            set_arg_next_use_info(assign->arg2, i);
          }
        }
      }
    }
  ...
  // X86 Codegen
  {
        ...
        li = li->next)
    {
      ...
      if(bb->label)
      {
        X86Stmt* label = x86_new_stmt(&x86_context, eX86StmtOpcode_label);
        label->operand1 = x86_make_id_operand(arena, bb->label->name);
      }
      if(bb == entry_point)
      {
        X86Stmt* mov = x86_new_stmt(&x86_context, eX86StmtOpcode_mov);
      ...
      for(int i = 0; i < bb->stmt_count; i++)
      {
        ...
        switch(ir_stmt->kind)
        {
          case eIrStmt_assign:
          {
            ...
            if(assign->op)
            {
              if(assign->op == eIrOp_index_source) // result = arg1[arg2]
              {
                ...
                eX86Location address_reg = get_best_available_register(&x86_context, ir_stmt);
                x86_emit_load_memory_address(&x86_context, address_reg, arg1->object);
                ...
                if(arg2->kind == eIrArg_object)
                {
                  ...
                  {
                    X86Stmt* mov_stmt = x86_new_stmt(&x86_context, eX86StmtOpcode_mov);
                    ...
                    set_exclusive_object_location(&x86_context, result->object, address_reg);
                  }
                  else // !is_register_location(arg2_loc)
                  {
                    X86Stmt* add_offset_stmt = x86_new_stmt(&x86_context, eX86StmtOpcode_add);
              ...
              else if(assign->op == eIrOp_index_dest) // result[arg2] = arg1
              {
                ...
                eX86Location address_reg = get_best_available_register(&x86_context, ir_stmt);
                x86_emit_load_memory_address(&x86_context, address_reg, result->object);
                ...
                if(arg2->kind == eIrArg_object && arg1->kind == eIrArg_object)
                {
                  ...
                  else if(is_register_location(arg2_loc) && !is_register_location(arg1_loc))
                  {
                    ...
                    {
                      save_object_to_memory(&x86_context, arg2_loc, arg2->object);
                    }
                    delete_object_from_location(&x86_context, arg2->object, arg2_loc);
                    ...
                    x86_emit_load_object_into_register(&x86_context, address_reg, arg1_loc, arg1->object);
                    add_object_to_location(&x86_context, arg1->object, arg1_loc);
                    ...
                    {
                      x86_emit_load_object_into_register(&x86_context, arg2_loc, eX86Location_memory, arg2->object);
                      add_object_to_location(&x86_context, arg2->object, arg2_loc);
                    }
                  }
                  else if(!is_register_location(arg2_loc))
                  {
                    ...
                    if(!is_register_location(arg1_loc))
                    {
                      arg1_loc = get_best_available_register(&x86_context, ir_stmt);
                      x86_emit_load_object_into_register(&x86_context, arg1_loc, eX86Location_memory, arg1->object);
                      add_object_to_location(&x86_context, arg1->object, arg1_loc);
                    }
                ...
                else if(arg2->kind == eIrArg_object && arg1->kind == eIrArg_constant)
                {
                  ...
                  if(!is_register_location(arg2_loc))
                  {
                    arg2_loc = get_best_available_register(&x86_context, ir_stmt);
                    x86_emit_load_object_into_register(&x86_context, arg2_loc, eX86Location_memory, arg2->object);
                    add_object_to_location(&x86_context, arg2->object, arg2_loc);
                  }
              ...
              else
              {
                ...
                {
                  eX86Location store_reg = select_result_register(&x86_context, ir_stmt);

                  if(arg1->kind == eIrArg_object)
                  {
                    if(!is_object_in_location(arg1->object, store_reg))
                    {
                      eX86Location arg1_loc = lookup_object_location(arg1->object);
                      assert(store_reg != arg1_loc);
                      x86_emit_load_object_into_register(&x86_context, store_reg, arg1_loc, arg1->object);
                    }

                    X86Stmt* x86_stmt = x86_new_stmt(&x86_context, conv_ir_op_to_x86_opcode(assign->op, assign->op_type));
                  ...
                  {
                    x86_emit_load_constant_into_register(&x86_context, store_reg, &arg1->constant);
                  ...
                  set_exclusive_object_location(&x86_context, result->object, store_reg);
                  discard_all_assign_args(&x86_context, assign, store_reg);
                }
                else // result = op arg1
                  fail(0);
              }
            ...
            else // result = arg1
            {
              if(arg1->kind == eIrArg_object)
              {
                ...
                if(is_register_location(arg1_loc))
                {
                  ...
                  discard_all_assign_args(&x86_context, assign, arg1_loc);
                }
              ...
              else if(arg1->kind == eIrArg_constant)
              {
                ...
                else
                {
                  ...
                  {
                    eX86Location store_loc = select_result_register(&x86_context, ir_stmt);
          ...
          case eIrStmt_cond_goto: // if arg1 relop arg2 goto L
          {
            ...
            eX86Location dest_loc = select_result_register(&x86_context, ir_stmt);
            if(arg1->kind == eIrArg_object)
            {
              if(!is_object_in_location(arg1->object, dest_loc))
              {
                eX86Location arg1_loc = lookup_object_location(arg1->object);
                assert(arg1_loc != dest_loc);
                x86_emit_load_object_into_register(&x86_context, dest_loc, arg1_loc, arg1->object);
              }
            }
            else if(arg1->kind == eIrArg_constant)
            {
              eX86Location dest_loc = select_result_register(&x86_context, ir_stmt);
              x86_emit_load_constant_into_register(&x86_context, dest_loc, &arg1->constant);
            }
            else assert(0);

            X86Stmt* cmp_stmt = x86_new_stmt(&x86_context, cmp_opcode);
            ...
            X86Stmt* jump_stmt = x86_new_stmt(&x86_context, conv_ir_op_to_x86_opcode(cond_goto->relop, cond_goto->op_type));
      ...
      save_all_registers_to_memory(&x86_context, true);
    }
    ...
    str_printfln(x86_text, "align %d", MACHINE_WORD_SIZE);
